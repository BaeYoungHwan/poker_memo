...
- 작업 단위가 안정화되면(기능 완성, Phase 완료 등) `develop` → `master` PR을 생성해 병합한다
- `master`는 "배포 가능한 상태"만 유지 — 직접 커밋·푸시 금지 (`permissions.deny`로 차단)
...
3. `/PR` 스킬로 `develop` → `master` PR 생성
4. PR 리뷰(셀프 리뷰 또는 sub-agent-review) 통과 후 머지
5. 머지 후 `develop`을 `master`와 동기화 (`git pull origin master` 또는 `git merge master`)
...
| 대안 | 문제 | 이 구조의 장점 |
|------|------|----------------|
| main 직접 커밋 (SCALE=1 기본) | 실수한 커밋이 곧바로 배포 기준에 반영됨 — 되돌리기 번거로움 | PR 단계에서 한 번 더 점검 가능, master는 항상 안정 상태 유지 |
